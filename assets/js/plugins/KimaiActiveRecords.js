...
export default class KimaiActiveRecords extends KimaiPlugin {
    ...
    setEntries(entries) {
        ...
        for (let timesheet of entries) {
            htmlToInsert +=
                    `<li>` +
                        `<a id="stopTimer" onclick="functionLoad()" href="${ this.attributes['href'].replace('000', timesheet.id) }" data-event="kimai.timesheetStop kimai.timesheetUpdate" class="api-link" data-method="PATCH" data-msg-error="timesheet.stop.error" data-msg-success="timesheet.stop.success">` +
                            `<div class="pull-left">` +
